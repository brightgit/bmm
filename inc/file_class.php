...
*/
	...
	public function open_visits( $action = "write", $log_info = FALSE ){
		...
		//echo $this->folder_prefix.$this->folder.'/visits/'.$this->filename;
		$this->conn_file = $this->folder_prefix.$this->folder.'/visits/'.$this->filename;
		if ( file_exists( $this->folder_prefix.$this->folder.'/visits/'.$this->filename ) ) {
			$this->conn = @fopen($this->folder_prefix.$this->folder.'/visits/'.$this->filename, 'a'); // or die("Não foi possível abrir ficheiro.");
		}else{
			$this->conn = @fopen($this->folder_prefix.$this->folder.'/visits/'.$this->filename, 'w'); // or die("Não foi possível criar ficheiro.");
		}

	}

	public function open_clicks( $action = "write", $log_info = FALSE ){
		...
		if ($query) {
			...
			$this->filename = date("Ymd", strtotime( $newsletter->date_in ) ).'-'.$newsletter->id.'.csv';
			//echo $this->folder_prefix.$this->folder.'/clicks/'.$this->filename;
			$this->conn_file = $this->folder_prefix.$this->folder.'/clicks/'.$this->filename;
			if ( file_exists( $this->folder_prefix.$this->folder.'/clicks/'.$this->filename ) ) {
				if( $action == "write" ) { $mode = 'a'; }elseif( $action == 'read' ){ $mode = "r"; }else{ die("Modo não definido."); }
				$this->conn = fopen($this->folder_prefix.$this->folder.'/clicks/'.$this->filename, $mode) or die("Não foi possível abrir ficheiro.");
			}else{
				if( $action != "write" ) { return false; }
				$mode = "w";
				$this->conn = fopen($this->folder_prefix.$this->folder.'/clicks/'.$this->filename, $mode) or die("Não foi possível criar ficheiro - file_class 126.");
			}
		}
		else
			echo "Something went wrong - line 101, file_class.php";
